...
export default function Page() {
    ...
    const [jumlahMerk, setJumlahMerk] = useState(0);
    ...
    const deleteMerkById = async (id) => {
        try {
            await fetch(`/api/merk?id=${id}`, {
                method: "DELETE",
                headers: { "Content-Type": "application/json" }
            });

            setMessage("Data Merk Berhasil Di Hapus");
            setTimeout(() => {
                setMessage("");
                redirect("/dashboard/bobot-merk");
            }, 3000);
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <div className="w-full h-screen select-none">
            ...
            <div className="w-full flex justify-between items-center">
                <h1 className="text-xl uppercase font-bold select-none">Halaman Bobot Merk</h1>
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <Link href={"/dashboard/bobot-merk/create"} className="bg-blue-500 hover:bg-blue-700 text-white  py-2 px-4 flex items-center gap-2.5">
                        <MdAdd className="text-xl" />Tambah Data Merk
                    </Link>
                </div>
            </div>
            ...
            <table className="w-full mt-4">
                ...
                <tbody>
                    ...
                    ) : bobotMerk.map((bobotMerk, index) => (
                        <tr key={bobotMerk.id}>
                            <td className="text-center p-2 border border-slate-900">{index + 1}</td>
                            <td className="p-2 border border-slate-900">{bobotMerk.name}</td>
                            <td className="text-center p-2 border border-slate-900">{bobotMerk.bobot}</td>
                            <td className="p-2 border border-slate-900 flex items-center gap-2.5">
                                <Link href={`/dashboard/bobot-merk/${bobotMerk.id}`}>
                                    <button className="bg-blue-500 hover:bg-blue-700 text-white  py-2 px-4 flex items-center gap-2.5"> Edit </button>
                                </Link>
                                <button onClick={() => deleteMerkById(bobotMerk.id)} className="bg-red-500 hover:bg-red-700 text-white  py-2 px-4 flex items-center gap-2.5"> Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            <table className="w-full mt-4">
                <thead>
                    <tr>
                        <th className="text-left p-2 border border-slate-900">Jumlah merk</th>
                ...
                <tbody>
                    <tr>
                        <td className="p-2 border border-slate-900">{jumlahMerk}</td>
            ...
            <h1 className="text-xl uppercase font-bold select-none my-4 underline underline-offset-4">Hasil Perhitungan AHP Bobot Merk</h1>
                    ...
                    {hasilAHP.length === 0 ? (
                        ...
                        hasilAHP.sort((a, b) => b.prioritas - a.prioritas).map((item, index) => (
                            <tr key={`${item.id}-${index}`}>
                                <td className="text-center p-2 border border-slate-900">{index + 1}</td>
                                <td className="p-2 border border-slate-900">{item.merk}</td>
