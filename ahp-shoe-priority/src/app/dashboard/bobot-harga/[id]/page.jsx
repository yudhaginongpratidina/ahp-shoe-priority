...
export default function Page({ params }) {
    ...
    const getBobotHargaById = async () => {
        try {
            const response = await fetch(`/api/harga?id=${params.id}`, {
                method: "GET",
                headers: {
    ...
    const updateBobotHargaById = async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                setMessage("");
                router.push("/dashboard/bobot-harga");
            }, 1000);
        } catch (error) {
            console.log(error);       
        }
                ...
                {harga.map((harga) => (
                    <div key={harga.id}>
                        ...
                        <div className="flex gap-2">
                            ...
                            <ButtonComponent type="button" name="Kembali" onClick={() => router.push("/dashboard/bobot-harga")} />
                        </div>
                    </div>
                ))}
            </form>
